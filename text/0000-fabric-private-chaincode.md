...
FPC Client SDK extension: This extends the NodeSDK (or Go SDK) with extra functionality that allows users to write end-to-end secure FPC-based applications. In particular, the client SDK extension will provide these core functions: 1) FPC transaction proposal creation (including transparent encryption of arguments) 2) FPC transaction proposal response validation and decryption of the result; 3) FPC deployment/setup functionality. The encryption of arguments in the proposal is performed by the Client SDK "under the covers" without requiring any special action by the administrator.
...
While the management API for Fabric is mostly unchanged, some modifications are needed for FPC to work. These are detailed separately in this document: [Link](https://github.com/hyperledger-labs/fabric-private-chaincode/blob/master/docs/design/fabric-v2%2B/fpc-management.md)

## FPC Chaincode Library / Shim

As described above, FPC replicates most of the programming model used in the stnadard Fabric Go shim in the C++ based Chaincode Library / Shim. These details are documented separately in the Shim header file itself: [Link](https://github.com/hyperledger-labs/fabric-private-chaincode/blob/master/ecc_enclave/enclave/shim.h)
